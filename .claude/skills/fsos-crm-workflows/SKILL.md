...
The native comms platform (Slices 1–6) extends the CRM in-place under `src/lib/comms/` and `/app/comms` — it is **not** a parallel marketing platform, and GoHighLevel stays **frozen** (not extended, not removed). Campaign/enrollment plumbing lives here; the *send-path compliance* details live in **twilio-a2p-compliance**.
...
- **Data model & RLS:** `docs/data-guardrails.md`; schema `009_aggregate_root_core.sql`, `010_rls_guardrails.sql`, later feature migrations, and comms-native `049`–`057`.
- **Native comms campaigns/enrollments:** `src/lib/comms/campaign.ts`, `campaign-run.ts`, `conversation-mode.ts`, `simulation.ts`; `/app/comms`. ADRs 013–021; slice docs `docs/comms-native/`.
