...
- **NIGO** ("Not In Good Order" defect-prevention) is a **separate project and is OUT OF SCOPE.** Do not create any NIGO module, agent, report, category, import, score, or cross-link anywhere. The legitimate application-tracking work lives in **Case Management OS** (`/app/cases`), which covers applications, submission tracking, underwriting, carrier requirements, documents, status/issue tracking, service requests, and case timelines — with NO NIGO functionality.
