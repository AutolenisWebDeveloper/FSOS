...
**Campaign timeline ribbon** (`components/comms/campaign/CampaignTimelineRibbon`) — the signature element on the three campaign detail screens: a percentage-positioned, RSC-safe ribbon (no SVG, no motion) that shows a campaign's *shape* — a cadence row of channel-colored touch ticks over the day axis, a neutral dashed acceleration marker, and (Life Conversion + Pipeline Win-Back only) a phase-banded **active-book** row whose brand-blue (`primary`) fill height is each phase's share of the live enrollments. All geometry is the pure, unit-tested `lib/comms/campaign-timeline.ts`. **Channel → categorical hue convention** (reuse for any future channel visualization so channels read the same everywhere): email → `chart-2` (cyan), sms → `chart-4` (violet), ai_conversation → `chart-3` (emerald), advisor_outreach → `chart-8` (slate — the human touch reads neutral). Brand blue (`chart-1`/`primary`) is reserved for the active-book fill so a channel and the live-enrollment measure never read as the same thing. Boldness is spent only on the active-book fill; the cadence, bands, and axis stay quiet. The Schedule table remains the authoritative, fully accessible per-touch record — the ribbon is the overview.
