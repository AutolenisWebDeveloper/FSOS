...
**TRAIGA 2026 / AI self-identification** — AI-driven conversations identify themselves as an automated assistant **in the message copy** (every playbook opener says so; enforced by `tests/lifecycle-campaign-messaging.test.mjs`). FSOS does not append a separate "generated by an AI assistant" footer — removed 2026-08-06 at the FSA's direction. Every SMS still carries the carrier-required `Reply STOP to opt out.` footer, appended by the dispatcher.
