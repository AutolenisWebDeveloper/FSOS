...
## Decision
Every outbound automated communication routes through **one dispatcher** that enforces, in order, blocking on first failure: (1) channel consent, (2) quiet hours (recipient-local, 9am–8pm floor), (3) DNC, (4) approved template or approved AI policy, (5) not an individualized securities recommendation, (6) not `is_security`-flagged, (7) no other FFS/Farmers/carrier/state/federal block. It applies TCPA prior-express-consent and A2P 10DLC requirements, appends the carrier-required `Reply STOP to opt out.` footer to every SMS, logs every send/block, tracks delivery, and escalates blocked sends rather than dropping them silently. AI self-identification lives in the message copy (every AI playbook opener), not in an appended footer — the former "generated by an AI assistant" footer was removed 2026-08-06 at the FSA's direction (see `CLAUDE.md` §12).
