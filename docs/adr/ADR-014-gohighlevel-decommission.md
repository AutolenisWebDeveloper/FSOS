...
**D4 — Schema retirement (deferred, additive-safe).** **Do not drop GHL ID columns in the same PR as code removal.** Retire schema only in a later, separate migration, after the reconciliation report is signed off, a full backup exists, and a rollback is tested. The forward migration and its tested rollback are **already drafted and staged** (not yet applied, and deliberately **not** in `supabase/migrations/` so a chain replay cannot execute D4 early):

- `docs/comms-ghl-migration/prepared/049_ghl_schema_retirement.sql` — the forward migration (drop 12 indexes, comment the 19 retained provenance columns, export-then-drop the two upload tables), carrying **PRECONDITION 0** (a live-schema drift check, required because `comment on column` has no `if exists` and migrations here are applied by hand via the Supabase SQL Editor) and a **RENUMBER-AT-EXECUTION** note (`049` will likely be taken by D1; re-derive the next free number and check for collision at promotion).
- `docs/comms-ghl-migration/prepared/049_ghl_schema_retirement_rollback.sql` — the paired structural rollback (recreate indexes + upload tables from their `002`/`003`/`004`/`023` definitions; data restored separately from the export/backup).

Concrete SQL guidance (implemented by the staged files above):

- **Migration files are NEVER edited or replaced — retirement happens only via a new final migration.** This applies to *every* migration file, not just `002`/`003`/`004`/`023`. Production has **already applied each migration by name**; Supabase records the applied set by filename, so an edited or renamed file **never re-runs in production** — it only diverges from what CI (`npm run test:rls`, `scripts/migrate.mjs`, which replay the whole chain from scratch) applies to a fresh database. Editing a migration therefore creates **silent prod/CI schema drift**: CI is green against schema X while production runs schema Y. The only safe change is a **new forward-only migration** appended to the chain. The next free number is **`049_ghl_schema_retirement.sql`** (`045`–`048` are already taken: `045_opportunity_source`, `046_opportunity_contact`, `047_opportunity_policy`, `048_appointment_lifecycle`).
...
- **Never edit or replace an existing migration file.** Production applied each by name; an edited file never re-runs in prod and silently drifts from CI. All schema change is a new forward-only migration.
...
- `docs/comms-ghl-migration/prepared/049_ghl_schema_retirement.sql` + `…_rollback.sql` (staged D4 forward migration + tested-rollback draft; not in `supabase/migrations/` until D4 is authorized)
