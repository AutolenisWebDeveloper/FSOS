...
**4. Migration 054 — additive schema.**
- `consents.purpose` (nullable): NULL = the existing channel-wide consent (backward-compatible); a purpose-scoped row grants/revokes one purpose. The old `unique(member_id, channel)` is replaced by two partial unique indexes so channel-wide and purpose-scoped rows coexist. `consent_ledger` is **untouched** — it stays append-only evidence, never an enforcement store (§9).
- `comm_messages.purpose` (nullable): records each send's purpose for frequency counting + analytics.
