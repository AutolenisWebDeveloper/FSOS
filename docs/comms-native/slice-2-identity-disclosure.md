...
| **Editable, approval-gated wording** | `comm_identity_config` (mig 051): full/abbreviated templates + Farmers role label + inactivity window + version + approval. Seeded as a **config default** (`is_assumption=true`, gold badge) in `draft` — nothing auto-discloses until approved (§4.3). |
