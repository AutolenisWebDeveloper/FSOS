...
| **The client's own policy** | Their policy or policy number, death benefit, cash value, face amount, beneficiaries, their coverage amount, riders, lapse status | A specific claim about a specific contract. FSOS should not assert policy facts unverified. |
...
| **Complaints and disputes** | Complaints, grievances, attorneys, lawsuits, disputes, refunds | Must reach a person immediately, with a record. |
...
The one situation the subject list is doing real work is where a scheduling invitation is
itself the wrong response — a complaint, or a message containing a Social Security number. Those
are on the list.
...
## 7. Questions worth a compliance answer

1. Are the twelve subjects in §3 the right set, and is anything missing that should never get
   an automated reply?
2. Is a scheduling invitation an acceptable automated response to a *sensitive but
   unrecognised* question (§6), or should the automation stay silent unless the subject is
   positively recognised as safe? The second option is available and is materially more
   restrictive — it would mean most inbound messages wait for Markist.
