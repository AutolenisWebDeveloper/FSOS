...
| FSOS project skills | ✅ All installed | `fsos-crm-workflows`, `fsos-security-audit`, `farmers-brand-website`, `twilio-a2p-compliance`. (`fsos-nigo-intelligence`, `finra-rule-ingestion`, `rightbridge-pdf-analysis` removed — ADR-040.) |
...
> **Superseded (2026-08-28):** this module was excised at the owner's direction — see
> `docs/adr/ADR-040-compliance-intelligence-excision.md`. The finding below remains an
> accurate description of HEAD `7d2d44d`; the files it cites no longer exist. The `036`/`037`
> tables it describes are retained but orphaned.
...
| Knowledge | `src/lib/knowledge/` | Retrieval + citation grounding. (`src/lib/compliance/intelligence.ts` removed — ADR-040.) |
