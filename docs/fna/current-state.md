...
- **Migrations** — this figure was a point-in-time note (then "run through 048"). As built, the FNA data model shipped as **`060_fna_data_model.sql`**, recommendations as **`062_fna_recommendations.sql`**, and the FNA performance indexes as **`066_fna_performance_indexes.sql`** (renumbered from 064 to resolve a version collision with the Social module's `064_social_schedule_retry.sql`; the 049 base was superseded by parallel comms/social migrations). New FNA migrations continue after the current head.
