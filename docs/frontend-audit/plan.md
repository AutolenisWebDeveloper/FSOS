...
- **Part 1 shipped:** `robots:index` + canonical `/workshops` in sitemap (H6 ✅); branded `opengraph-image.tsx` via `next/og` (H7 OG ✅); `/403` dead-end → `mailto` (H8 ✅); `[slug]` NAP sourced from the `CONTACT` single-source (M9 NAP ✅).
- **Part 2 shipped:** `apple-icon.tsx` via `next/og` (H7 ✅); `[slug]` split into server `page.tsx` (`metadata`, noindex) + `ReferralClient.tsx` (M9 ✅); `.msite` marketing palette formally sanctioned in DESIGN.md §6.5 with a no-inline-hex rule (H5 ✅); inline hexes → `.msite` classes (L2 ✅); `SiteHeader` + `SiteFooter` internal links → `next/link` (L6 partial — 56→22 warnings). Verified type-check/lint/test/build green.
- **Part 3 shipped:** logo intrinsic `width/height` — footer carrier logo + emblem (L7 ✅); remaining public `<a>`→`Link` — legal pages, consent forms, workshop detail pages, so **public `no-html-link` warnings = 0** (L6 ✅, only private-app `WorkshopStatusControl` remains → Slice 2); auth-form review — inputs already carry correct `autoComplete`/`inputMode`/`pattern`, full states, a11y, and a responsive `AuthShell`, so **no blind restyling** was done. Verified type-check/lint/test/build green.
- **Still open (genuinely needs a running browser):** auth-screen *visual* fine-tuning + a full responsive/a11y capture pass at the breakpoint matrix — deferred to a session with browser access; no objective code defect remains on these surfaces.
