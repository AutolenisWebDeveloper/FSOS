...
// FFS Directory (Contacts & Contact 360 redesign §1/§5) — the internal Farmers
// resource list, explicitly NOT the CRM. Renamed from "FFS Contacts" and moved to
// its own /app/directory route so the terminology no longer collides with the CRM
// Contacts/Household entity. Read-only view of the SAME config-driven
// `ffs_contacts` table that /super/config/ffs-contacts edits — contacts are
// configuration, never hard-coded. No mutation here, so no audit event.
...
export default async function FfsDirectoryPage() {
  await requireRole('fsa', '/app/directory')
  ...
  if (!res.ok) {
    ...
    body = (
      <EmptyState
        ...
        description="The FFS Directory is managed as configuration under Super → Config → FFS Contacts."
      />
    )
  } else {
    body = (
  ...
  return (
    <ListShell
      title="FFS Directory"
      description="Quick access to Farmers Financial Solutions desks and specialists — an internal resource list, not the CRM. Managed as config; verify before relying on any number."
      breadcrumb={[{ label: 'FSA', href: '/app' }, { label: 'FFS Directory' }]}
    >
      {body}
    </ListShell>
  )
