...
// In-Force Book import (FNWL district review → App B aggregate root). Two modes:
//   preview — parse + summarize + count what already exists; NO writes.
//   commit  — idempotent load: serving agents → agency_partnerships, owners →
//             households (+ owner/insured members for new households), each policy
//             → household_policies (variable → is_security). Dedupe by selecting
//             existing provenance keys then inserting only what's new, so a
//             re-import adds nothing. RBAC-gated + audited.
export async function POST(req: NextRequest) {
  ...
  // Distinct entities from the book.
  ...
  // What already exists (drives both the preview plan and the commit filter).
  ...
  // ── COMMIT ──────────────────────────────────────────────────────────────
  try {
  // 1. New serving agents → agency_partnerships.
  const newAgencyRows = agentNos
    .filter((no) => !existingAgentSet.has(no))
    .map((no) => ({ fnwl_serving_agent_no: no, agency_name: agents.get(no)!, owner_name: agents.get(no)!, status: 'producing' }))
  ...
  const newHouseholdRows = newHouseholdKeys.map((k) => {
    ...
    return {
      book_owner_key: k,
      primary_name: p.owner_name,
      address: p.owner_address,
      city: p.owner_city,
      state: (p.owner_state || 'TX').slice(0, 2),
      zip: (p.owner_zip || '').slice(0, 10) || null,
      referring_agency_id: p.serving_agent_no ? agencyId.get(p.serving_agent_no) ?? null : null,
    }
  })
  ...
  // 3. Members for NEW households only (owner + insured when different).
  const memberRows: Array<Record<string, unknown>> = []
  for (const k of newHouseholdKeys) {
    const p = households.get(k)!
    const hid = householdId.get(k)
    if (!hid) continue
    memberRows.push({ household_id: hid, full_name: p.owner_name, relationship: 'owner', phone: p.owner_phone })
    if (p.insured_name && p.insured_name.trim().toLowerCase() !== p.owner_name.trim().toLowerCase()) {
      memberRows.push({ household_id: hid, full_name: p.insured_name, relationship: 'insured' })
    }
  }
  await insertChunked(db, 'household_members', memberRows)

  // 4. New policies → household_policies.
  const newPolicyRows = policyNumbers
    .filter((pn) => !existingPolSet.has(pn))
    .map((pn) => {
      ...
      return {
        ...
        accumulation_value: p.accumulation_value,
        effective_date: p.issue_date,
        conversion_deadline: p.conversion_date,
        source_system: 'fnwl',
  ...
  return NextResponse.json({
    ...
    plan,
    committed: {
      agencies_new: newAgencyRows.length,
      households_new: newHouseholdKeys.length,
      members_added: memberRows.length,
      policies_added: newPolicyRows.length,
    },
  })
  } catch (e) {
...
// ── helpers ────────────────────────────────────────────────────────────────
// Set of key values that already exist in `table` among the given `values`.
// fnwlOnly restricts to source_system='fnwl' (policy dedupe within our imports).
