...
// intersection; every action is audit-logged with prev→next state + reason. Because only an Active
// campaign dispatches (states.canDispatch), a global Disable/Emergency-Stop halts every channel +
// the enrollment sweep in one place. Resume behavior + replay policy may be overridden per-action.
