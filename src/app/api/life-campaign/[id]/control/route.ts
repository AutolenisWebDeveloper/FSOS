...
import { applyControl, CONTROL_ROLES } from '@/lib/life-campaign/controls'
...
const ControlSchema = z.object({
  ...
  // Resume strategy overrides (§4b). Absent → the safe default (only_admin_paused + skip: resume in
...
// Operational controls (§4b): enable / pause / resume / disable / emergency-stop / archive.
// Restricted to ops/admin/super/fsa via the existing RBAC role-intersection; every action is
// audit-logged with prev→next state + reason, and disable/emergency-stop returns the
// deadline-exposure warning for this deadline-sensitive campaign.
