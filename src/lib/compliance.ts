// src/lib/compliance.ts
// FSOS Compliance Constants — do not modify without legal review

export const FINRA_DISCLAIMER =
  'For educational and informational purposes only. Not a product recommendation ' +
  'or suitability determination. Requires licensed FSA review per FINRA Reg BI.'

// Appended by the dispatcher to EVERY outbound SMS (src/lib/comms/dispatcher.ts).
// Carries the CTIA / A2P 10DLC opt-out keyword only. The former AI-disclosure sentence
// ("This message was generated by an AI assistant.") was removed at the FSA's direction
// on 2026-08-06: it is not required of a private business under TRAIGA (HB 149, whose
// disclosure mandate reaches government and healthcare interactions), and it consumed
// ~47 chars of segment budget on every send. AI self-identification still happens in the
// message copy itself — every AI conversation opener says "automated assistant" and that
// is asserted by tests/lifecycle-campaign-messaging.test.mjs.
// The "Reply STOP" keyword is a carrier requirement and may NOT be removed.
export const SMS_OPT_OUT_FOOTER = 'Reply STOP to opt out, HELP for help.'

export const AI_PERMITTED_ACTIONS = [
  'Educate on financial concepts',
  'Qualify leads and gather information',
  'Schedule and confirm appointments',
  'Send reminders and follow-ups',
  'Route conversations to Markist',
  'Escalate to licensed agent',
] as const

export const AI_PROHIBITED_ACTIONS = [
  'Recommend specific products',
  'Make suitability determinations',
  'Give investment advice',
  'State a product is right for a client',
  'Act as a licensed agent in any way',
  'Make securities recommendations',
] as const

// Half-open bands [minGdc, maxGdc): each tier's maxGdc equals the next tier's
// minGdc so there is NO gap between tiers. (The previous inclusive maxGdc of
// 14999/54999 left the ranges 14999–15000 and 54999–55000 unowned, so a
// fractional-dollar rolling total like $54,999.50 matched no tier and silently
// fell back to Tier 1 / 40%.)
export const GDC_TIERS = [
  { tier: 1, label: 'Tier 1', minGdc: 0,     maxGdc: 15000,    rate: 0.40, rateLabel: '40%', range: 'Under $15,000' },
  { tier: 2, label: 'Tier 2', minGdc: 15000, maxGdc: 55000,    rate: 0.60, rateLabel: '60%', range: '$15,000 – $54,999' },
  { tier: 3, label: 'Tier 3', minGdc: 55000, maxGdc: Infinity, rate: 0.80, rateLabel: '80%', range: '$55,000+' },
] as const

export function getTier(rollingGdc: number): typeof GDC_TIERS[number] {
  // Round to whole cents first: a rolling GDC is summed from many numeric(12,2)
  // cases in JS floating point, so a true $55,000.00 can land as 54999.9999998
  // and demote a producer a full tier. Then select the highest tier whose lower
  // threshold is met — gap-free by construction.
  const g = Number.isFinite(rollingGdc) ? Math.round(rollingGdc * 100) / 100 : 0
  for (let i = GDC_TIERS.length - 1; i >= 0; i--) {
    if (g >= GDC_TIERS[i].minGdc) return GDC_TIERS[i]
  }
  return GDC_TIERS[0]
}

export const FFS_CONTACTS = [
  { role: 'Sales Desk', name: 'Case consultations, products, training', tel: '(866) 888-9739', ext: '→ 3→3', hours: 'Mon–Fri 7AM–5PM PT' },
  { role: 'FSD Central', name: 'Matt Anderson', tel: '(818) 584-0264', email: 'matthew.1.anderson@farmersinsurance.com' },
  { role: 'Internal Wholesaler', name: 'Ando Agamalian', tel: '(818) 584-0205', email: 'ando.agamalian@farmersinsurance.com' },
  { role: 'Compliance TX', name: 'Ryan Anderson', tel: '(253) 242-0597', email: 'ryan.anderson@farmersinsurance.com' },
  { role: 'OSJ Principal', name: 'Lora Brandt', tel: '(818) 584-0199', email: 'lora.brandt@farmersinsurance.com' },
  { role: 'Commissions', name: 'Finance Team', tel: '(866) 888-9739', ext: '→ 6', email: 'usw_ffs_accounting@farmersinsurance.com' },
] as const
