...
-- ─────────────────────────────────────────────────────────
-- 1. GDC tiers — rolling-12mo Gross Dealer Concession → FSA payout %.
...
--    slug gives a stable idempotent seed/upsert key.
-- ─────────────────────────────────────────────────────────
create table if not exists ffs_contacts (
  id            uuid primary key default gen_random_uuid(),
  slug          text not null unique,             -- stable key: 'fsd-central-tx'
  role          text not null,                    -- "FSD — Central (TX)"
  name          text,                             -- person (null for a desk line)
  phone         text not null,
  hours         text,                             -- "Mon–Fri 7AM–5PM PT"
  note          text,                             -- "Opt 3 → 3"
  active        boolean not null default true,
  sort          integer not null default 0,
  created_at    timestamptz not null default now(),
...
-- GDC tiers: FSA production tracking + super/compliance oversight + admin.
...
insert into ffs_contacts (slug, role, name, phone, hours, note, sort) values
  ...
  ('sales-desk',          'Sales Desk',             null,               '(866) 888-9739', 'Mon–Fri 7AM–5PM PT',  'Opt 3 → 3',  5)
