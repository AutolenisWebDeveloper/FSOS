-- ─────────────────────────────────────────────────────────
-- Migration: 099_pipeline_winback_messaging_v2
--
-- Replaces ALL customer-facing Pipeline Win-Back copy (ADR-031) with a rebuilt
-- lifecycle-marketing message set. Migration 084 (applied) is NOT modified — this
-- migration UPDATEs the same comm_templates rows IN PLACE, which is the repo's
-- established versioning idiom (api/comms/templates/[id]/route.ts and
-- scripts/build-email-templates.ts both bump `version` and reset approval on any
-- body change). Template IDs are preserved, so pipeline_winback_touches keeps
-- resolving the same rows and the 24-touch schedule/automation is untouched.
--
-- Every row lands at version 2 / approval_status='draft' with the prior approval
-- cleared. sendThroughGate() refuses an unapproved template, so nothing can
-- dispatch until a compliance reviewer approves it. This migration approves
-- nothing and activates nothing.
--
-- COPY CONTRACT (enforced by the send path, not by convention):
--   • Email bodies use the wrapMarketingEmailBody vocabulary (email-shell.ts):
--     a leading "Subject:" line (rendered as the H1), a "Preview:" line (inbox
--     preheader), "* " bullets, ONE "Label {{scheduling_link}}" CTA line (rendered
--     as a bulletproof button), and exactly ONE closer block ("Warm regards," /
--     "Sincerely,") which becomes the rich branded signature. The trailing
--     disclaimer matches DISCLAIMER_RE and is deliberately superseded by the
--     CAN-SPAM marketing footer at render time.
--   • SMS bodies carry NO opt-out text. dispatcher.ts:170 appends
--     TRAIGA_SMS_FOOTER ("This message was generated by an AI assistant. Reply
--     STOP to opt out.") to every SMS, which supplies both the TRAIGA AI
--     disclosure and the opt-out. The prior copy duplicated it on the wire.
--   • Merge tokens are restricted to the six proven-resolvable tokens used by the
--     campaign send path: first_name (cosmetic) and fsa_name, agency_name,
--     scheduling_link, advisor_phone, advisor_email (blocking-tier, resolved by
--     buildRecipientContext). No new token is introduced.
--
-- GUARDRAILS: every body is green-zone (educate / invite / schedule / follow up).
-- No product, carrier, coverage-amount, premium, or replacement recommendation;
-- no suitability determination; no invented Farmers/FNWL data, rate, or statistic;
-- no specific claim about the recipient's own policy or coverage status (ADR-020).
-- Every body is asserted recommendation-free by tests/pipeline-winback-messaging.test.mjs.
--
-- Idempotent (fixed IDs, deterministic values — re-run is a no-op).
...
It's {{fsa_name}} with {{agency_name}}. A while back you started looking into life insurance with us, and then things went quiet. That happens far more often than you would think.

I am not writing to nudge you toward anything. I am the life and financial services specialist who works alongside your Farmers agent, and part of my job is making sure nobody gets left in limbo on something they once cared about.
...
update comm_templates set
  name = 'Win-Back — SMS #1 — Still On Your List',
  body = $body$Hi {{first_name}}, it's {{fsa_name}} with {{agency_name}}. You looked into life insurance with us a while back, then things went quiet. Completely normal. Is it still on your list, or should I close it out?$body$,
  approval_status = 'draft', version = 2, approved_at = null, approved_by = null,
  updated_by = 'messaging-v2', updated_at = now()
...
update comm_templates set
  ...
  body = $body$Hi {{first_name}}, I'm the automated assistant for {{fsa_name}} at {{agency_name}}. You looked into life insurance with us a while back. Is now a better time to pick it back up, or should we close it out for now?$body$,
  approval_status = 'draft', version = 2, approved_at = null, approved_by = null,
  updated_by = 'messaging-v2', updated_at = now()
...
-- ── ROLLBACK (manual) ───────────────────────────────────────────────────────
--   Re-apply 084_pipeline_winback_seed.sql bodies and set version = 1 for the
