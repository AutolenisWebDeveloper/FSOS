...
try {
  ...
  section('1. Inbound message → thread, association, history, auto-reply attempt')
  {
    ...
    check('the dispatched reply carries the SMS opt-out footer (and no AI-disclosure)', () => {
      // The carrier-required opt-out footer must be present on every SMS. The former
      // AI-disclosure sentence ("…generated by an AI assistant.") was removed at the FSA's
      // direction on 2026-08-06 (SMS_OPT_OUT_FOOTER in src/lib/compliance.ts): it is not
      // required of a private business under TRAIGA (HB 149). Assert it stays gone so it
      // cannot silently creep back into the SMS length budget.
      ...
      assert.doesNotMatch(body, /AI assistant/i, `the AI-disclosure sentence was retired on 2026-08-06 and must not reappear: ${body}`)
    })
