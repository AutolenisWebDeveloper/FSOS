...
const { planResume, touchKind, TOTAL_TOUCHES } = require(join(out, 'resume.js'))
...
t('touches that came due while paused are recorded skipped, cadence jumps to the first future touch', () => {
  ...
  const r = planResume({ baselineDate: BASELINE, currentTouchNo: 2, today, replay: 'skip' })
...
t('no burst: resuming never returns more than one due touch (next_touch_at is a single future date)', () => {
  ...
  const r = planResume({ baselineDate: BASELINE, currentTouchNo: 1, today, replay: 'skip' })
...
t('resuming after the whole cadence is past-due skips the remainder and completes', () => {
  ...
  const r = planResume({ baselineDate: BASELINE, currentTouchNo: 5, today, replay: 'skip' })
...
t('a cursor already at the final touch completes with nothing skipped', () => {
  const r = planResume({ baselineDate: BASELINE, currentTouchNo: TOTAL_TOUCHES, today: dueOf(1), replay: 'skip' })
...
t('replay records NO skips and fires the next pending touch immediately (today), not late', () => {
  ...
  const r = planResume({ baselineDate: BASELINE, currentTouchNo: 2, today, replay: 'replay' })
...
t('replay on an exhausted timeline completes', () => {
  const r = planResume({ baselineDate: BASELINE, currentTouchNo: TOTAL_TOUCHES, today: dueOf(1), replay: 'replay' })
  assert.equal(r.nextTouchAt, null)
  assert.equal(r.complete, true)
})
...
t('touchKind returns the frozen schedule channel for each touch, null out of range', () => {
  assert.equal(touchKind(1), 'email')
  assert.equal(touchKind(2), 'sms')
  assert.equal(touchKind(7), 'advisor_outreach')
  assert.equal(touchKind(0), null)
  assert.equal(touchKind(999), null)
  assert.equal(TOTAL_TOUCHES, TOUCH_SCHEDULE.length)
})
